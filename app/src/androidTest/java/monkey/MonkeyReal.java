...
public class MonkeyReal {
    ...
    public MonkeyReal(Instrumentation instrumentation,Display display,PackageManager packageManager,String testPackage){
    ...
    private void commandLineReport(String reportName, String command) {
        ...
        try {
            ...
            if (mRequestBugreport) {
                logOutput =
                        new BufferedWriter(new FileWriter(new File(Environment.getExternalStorageDirectory(), reportName), true));
            }
...
//        Process.setArgV0("com.android.commands.monkey");
        ...
        int resultCode = (new MonkeyReal()).run(args);
...
//            mAm = mGetDefault.invoke(null, null);
            Method mSetActivityController = cActivityManagerNative.getMethod("setActivityController", Class.forName("android.app.IActivityController"));
            mSetActivityController.invoke(mAm, null);
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
    ...
    /*private boolean getMainApps() {
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                List<ResolveInfo> mainApps=inst.getContext().getPackageManager().queryIntentActivities(intent,0);
//                List<ResolveInfo> mainApps = mPm.queryIntentActivities(intent, null, 0, UserHandle.myUserId()).getList();
